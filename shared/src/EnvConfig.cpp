...
bool EnvConfig::load(const std::string& path) {
    std::ifstream f(path);
    if (!f) return false;
    std::string line;
...
std::string EnvConfig::get(const std::string& key, const std::string& default_value) const {
    if (const char* env = std::getenv(key.c_str())) return std::string(env);
    auto it = vars_.find(key);
    return it != vars_.end() ? it->second : default_value;
}

int EnvConfig::getInt(const std::string& key, int default_value) const {
    auto v = get(key, "");
    if (v.empty()) return default_value;
    try { return std::stoi(v); } catch (...) { return default_value; }
}

double EnvConfig::getDouble(const std::string& key, double default_value) const {
    auto v = get(key, "");
    if (v.empty()) return default_value;
